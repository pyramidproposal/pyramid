          ...
          const unsigned char *in, unsigned long inlen)
{
    ...
    for (i = 0; (i+1)*PYR_SHA256_OUTPUT_BYTES <= outlen; i++) {
        u32_to_bytes(inbuf + inlen, i);
        sha256(out, inbuf, inlen + 4);
        out += PYR_SHA256_OUTPUT_BYTES;
    }
    /* Until we cannot anymore, and we fill the remainder. */
    if (outlen > i*PYR_SHA256_OUTPUT_BYTES) {
        u32_to_bytes(inbuf + inlen, i);
        sha256(outbuf, inbuf, inlen + 4);
        memcpy(out, outbuf, outlen - i*PYR_SHA256_OUTPUT_BYTES);
    }
}
...
{
    unsigned char inbuf[inlen + 4];
    unsigned char outbuf[PYR_SHA512_OUTPUT_BYTES];
    unsigned long i;

    memcpy(inbuf, in, inlen);

    /* While we can fit in at least another full block of SHA512 output.. */
    for (i = 0; (i+1)*PYR_SHA512_OUTPUT_BYTES <= outlen; i++) {
        u32_to_bytes(inbuf + inlen, i);
        sha512(out, inbuf, inlen + 4);
        out += PYR_SHA512_OUTPUT_BYTES;
    }
    /* Until we cannot anymore, and we fill the remainder. */
    if (outlen > i*PYR_SHA512_OUTPUT_BYTES) {
        u32_to_bytes(inbuf + inlen, i);
        sha512(outbuf, inbuf, inlen + 4);
        memcpy(out, outbuf, outlen - i*PYR_SHA512_OUTPUT_BYTES);
    }
}
...
void seed_state(const unsigned char *pub_seed) {
    uint8_t block[PYR_SHA256_BLOCK_BYTES];
    size_t i;

    for (i = 0; i < PYR_N; ++i) {
        block[i] = pub_seed[i];
    }
    for (i = PYR_N; i < PYR_SHA256_BLOCK_BYTES; ++i) {
        block[i] = 0;
    }
