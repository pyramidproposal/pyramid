 ...
 */
static void expand_seed(unsigned char outseeds[PYR_WOTS_PK_BYTES],
                        const unsigned char inseed[PYR_N],
                        const unsigned char pub_seed[PYR_N],
 ...
 */
static void gen_chain(unsigned char out[PYR_N], const unsigned char in[PYR_N],
                      unsigned int start, unsigned int steps,
                      const unsigned char pub_seed[PYR_N], uint32_t addr[8]) {
  uint32_t i;
 ...
 */
static void wots_checksum(unsigned int csum_base_w[PYR_WOTS_LEN2],
                          const unsigned int msg_base_w[PYR_WOTS_LEN1]) {
  unsigned int csum = 0;
 ...
 */
static void chain_lengths(unsigned int lengths[PYR_WOTS_LEN],
 ...
 */
void wots_pkgen(unsigned char pk[PYR_WOTS_PK_BYTES],
                const unsigned char seed[PYR_N],
                const unsigned char pub_seed[PYR_N], uint32_t addr[8]) {
  uint32_t i;
 ...
 */
void wots_sign(unsigned char sig[PYR_WOTS_BYTES],
               const unsigned char msg[PYR_N], const unsigned char seed[PYR_N],
               const unsigned char pub_seed[PYR_N], uint32_t addr[8]) {
 ...
 * Writes the computed public key to 'pk'.
 */
void wots_pk_from_sig(unsigned char pk[PYR_WOTS_PK_BYTES],
                      const unsigned char sig[PYR_WOTS_BYTES],
                      const unsigned char msg[PYR_N],
                      const unsigned char pub_seed[PYR_N], uint32_t addr[8]) {
  unsigned int lengths[PYR_WOTS_LEN];
  uint32_t i;
 ...
 */
void gen_leaf_wots(unsigned char leaf[PYR_N],
                   const unsigned char sk_seed[PYR_N],
                   const unsigned char pub_seed[PYR_N], uint32_t wotspk_addr[8],
                   uint32_t ots_addr[8]) {
