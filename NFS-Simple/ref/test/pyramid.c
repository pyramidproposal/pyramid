...
static inline int t_sk_deleted_correctlyp(
...
static inline int t_mlen_correctp(size_t smlen) { return smlen == PYR_MLEN; }
  ...
  for (i = 0; (i < PYR_SIGNATURES) && !ret; i++) {
    ...
    switch (retval) {
      case 0:
        if (t_invalid_idxp(i)) {
          printf(
              "Invalid index should result in error return value -2, got %d.\n",
              retval);
          ret |= -1;
        } else if (t_last_idxp(i)) {
          printf(
              "Last valid signature index should return warning value -1, got "
              "%d.\n",
              retval);
          ret |= -1;
        }
      ...
      case -2:
        if (t_invalid_idxp(i)) {
          printf(
              "Unknown failure, return value %d correct,"
              "but this should be unreachable.\n",
              retval);
        } else if (t_last_idxp(i)) {
          printf(
              "Last index should give a valid signature return value -1, got "
              "%d.\n",
              retval);
          ret |= -1;
        } else {
        ...
        case -2:
          if (nr_fail_after_last_index == 0) {
            if (t_sk_deleted_correctlyp(sk)) {
              ...
              printf(
                  "Return values -2, -1 in succession, but sk was not deleted "
                  "correctly.\n");
              ret |= -1;
            }
          }
          if (nr_fail_after_last_index < (uint64_t)PYR_NR_FAIL) {
            if (t_sk_deleted_correctlyp(sk)) {
              ...
              if (nr_fail_after_last_index == (uint64_t)PYR_NR_FAIL) {
                printf(
                    "Correctness checked for %lu iterations,\n"
              ...
              printf(
                  "Return values -2, -1, ..., -1 in succession,"
