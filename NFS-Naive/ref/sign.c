...
int crypto_sign_seed_keypair(unsigned char *pk, unsigned char *sk,
  ...
  initialize_hash_function(sk + 3 * PYR_N, NULL);
...
static int last_idxp(uint64_t idx) { return idx == LAST_VALID_IDX; }

static int invalid_idxp(uint64_t idx) { return idx > LAST_VALID_IDX; }
