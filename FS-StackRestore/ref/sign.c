 ...
 * Format sk: [index || SK.Seed || SK.PRF || PK.Seed || PK.Root]
...
int crypto_sign_seed_keypair(unsigned char *pk, unsigned char *sk,
  ...
  unsigned char th_stack[PYR_TREE_HEIGHT * PYR_N];
  unsigned char next_seed[PYR_N];
  ...
  memcpy(sk, seed, (PYR_D + 1) * PYR_N);

  /* Initialize PUB_SEED. */
  memcpy(sk + (PYR_D + 2) * PYR_N, seed + (PYR_D + 1) * PYR_N, PYR_N);
  memcpy(pk + PYR_N, sk + (PYR_D + 2) * PYR_N, PYR_N);

  initialize_hash_function(sk + (PYR_D + 2) * PYR_N, NULL);

  /* Compute root node of the top-most subtree. */
  treehash(pk, auth_path, next_seed, th_stack, 0, sk + (PYR_D - 1) * PYR_N,
           pk + PYR_N, 0, top_tree_addr);
  memset(next_seed, 0, PYR_N);
  memcpy(sk + (PYR_D + 1) * PYR_N, pk, PYR_N);

  /* Zero treehash stacks to be safe. */
  memset(sk + PYR_SK_CORE_BYTES, 0, PYR_SK_STATE_BYTES);
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen, const uint8_t *m,
  ...
  /* The first of PYR_D sk seeds, one for each layer. */
  const unsigned char *sk_prf = sk + PYR_IDX_BYTES + PYR_D * PYR_N;
  const unsigned char *pub_root = sk + PYR_IDX_BYTES + (PYR_D + 1) * PYR_N;
  const unsigned char *pub_seed = sk + PYR_IDX_BYTES + (PYR_D + 2) * PYR_N;
  ...
  /* FS additions: */
  unsigned char *sk_seed = sk + PYR_IDX_BYTES;
  unsigned char *th_stack = sk + PYR_IDX_BYTES + (PYR_D + 3) * PYR_N;
  unsigned char wots_seed[PYR_N];
  unsigned char next_seed[PYR_N];
  unsigned int c = 1;
  ...
  *siglen = 0;
  ...
  for (i = 0; i < PYR_D; i++) {
    ...
    fsg(wots_seed, NULL, sk_seed, pub_seed, wots_addr);

    /* Compute a WOTS signature. */
    /* Initially, root = mhash, but on subsequent iterations it is the root
       of the subtree below the currently processed subtree. */
    wots_sign(sig, root, wots_seed, pub_seed, wots_addr);
    sig += PYR_WOTS_BYTES;

    /* Compute the authentication path for the used WOTS leaf. */
    /* FS: When c = 0, we do not overwrite the Treehash state,
       as we did not hop a leaf on the current layer. */
    treehash(root, sig, next_seed, th_stack, c, sk_seed, pub_seed, idx_leaf,
             wots_addr);

    sig += PYR_TREE_HEIGHT * PYR_N;

    /* FS: Advance if carry, set carry. */
    if (c && !is_last_idx) {
      memcpy(sk_seed, next_seed, PYR_N);
      c = idx_leaf == (UINT32_MAX >> (32 - PYR_TREE_HEIGHT));
    }

    sk_seed += PYR_N;
    th_stack += PYR_TREE_HEIGHT * PYR_N;
  }
