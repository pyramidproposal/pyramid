...
int main() {
  ...
  unsigned char input[TH1_INLEN];
  unsigned char output_c[PYR_N];
  unsigned char output_jazz[PYR_N];

  // Initialize variables for test
  random_test_bytes(input, TH1_INLEN);

  // Test

  shake256(output_c, PYR_N, input, TH1_INLEN);
  shake256_PYR_N_TH1_INLEN_jazz(output_jazz, input);

  for (i = 0; i < PYR_N; ++i) {
    if (output_c[i] != output_jazz[i]) {
      printf(
          "[!] Unit test "
          "failed.\nFunction:\tshake256_PYR_N_TH1_INLEN.\nReason:\toutput_c[%d]"
          " != output_jazz[%d] ==> %d != %d\n",
