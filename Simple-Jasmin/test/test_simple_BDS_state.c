...
int test_BDS_th_get_sdn(const unsigned char *sk) {
  ...
  memcpy(sdn_c, (state_c.treehash + TH_INDEX)->ref_next, PYR_N);
...
int test_BDS_th_set_sdn(const unsigned char *sk) {
  ...
  memcpy((state_c.treehash + TH_INDEX)->ref_next, node, PYR_N);
...
int main() {
  ...
  /* Stack size is 0 otherwise. */
#if PYR_TREE_HEIGHT > 1
  ...
  /* Ensure empty stack for push test (important when h/d = 2). */
  state_deserialize(sk_c, &state_c, STATE_TYPE);
  state_c.stack->size = 0;
  state_serialize(sk_c, &state_c, STATE_TYPE);
  /* Stack size is now 0. */
