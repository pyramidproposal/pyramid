...
int main() {
  ...
  // Test
  keccak_squeezeblocks(h_c, PYR_N / SHAKE256_RATE, state_c, SHAKE256_RATE);
  keccak_squeezeblocks_256_PYR_N_jazz(h_jazz, state_jazz);

  for (i = 0; i < PYR_N; ++i) {
    if (h_c[i] != h_jazz[i]) {
      printf(
          "[!] Unit test "
          "failed.\nFunction:\tkeccak_squeezeblocks_256_PYR_N.\nReason:\th_c[%"
          "d] != h_jazz[%d] ==> %d != %d\n",
          i, i, h_c[i], h_jazz[i]);
    }
  }
