...
int main() {
  ...
  unsigned char m_c[PRF_INLEN];
  unsigned char m_jazz[PRF_INLEN];
  ...
  random_test_bytes(m_c, PRF_INLEN);

  for (i = 0; i < PRF_INLEN; ++i) {
    m_jazz[i] = m_c[i];
  }

  // Test
  keccak_absorb(state_c, SHAKE256_RATE, m_c, PRF_INLEN, 0x1F);
  keccak_absorb_256_PRF_INLEN_jazz(state_jazz, m_jazz);

  for (i = 0; i < 25; ++i) {
    if (state_c[i] != state_jazz[i]) {
      printf(
          "[!] Unit test "
          "failed.\nFunction:\tkeccak_absorb_256_PRF_INLEN.\nReason:\tstate_c[%"
