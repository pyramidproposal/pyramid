...
int main() {
  ...
  unsigned char input[HPRG_INLEN];
  unsigned char output_c[PYR_N];
  unsigned char output_jazz[PYR_N];

  // Initialize variables for test
  random_test_bytes(input, HPRG_INLEN);

  // Test

  shake256(output_c, PYR_N, input, HPRG_INLEN);
  shake256_PYR_N_HPRG_INLEN_jazz(output_jazz, input);

  for (i = 0; i < PYR_N; ++i) {
    if (output_c[i] != output_jazz[i]) {
      printf(
          "[!] Unit test "
          "failed.\nFunction:\tshake256_PYR_N_HPRG_INLEN.\nReason:\toutput_c[%"
          "d] != output_jazz[%d] ==> %d != %d\n",
