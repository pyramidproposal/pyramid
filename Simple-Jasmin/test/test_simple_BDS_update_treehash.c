...
int test_BDS_update_treehash_tree() {
  int res = 0;
  uint32_t s, t;
  ...
  BDS_update_treehash_jazz(sk_psd, t, sk_jazz);

  /* serialize to write non-pointers */
  state_serialize(sk_c, &state_c, STATE_TYPE);
  res = cmp_BDS_state(sk_c, sk_jazz);
  if (res) {
...
int test_BDS_update_treehash_node() {
  int res = 0;
  size_t j;
  uint32_t s, t;
  unsigned char sk_psd[PYR_N];
...
int test_BDS_update_treehash_node_random(unsigned int nr_test) {
  int res = 0;
  size_t j;
  uint32_t s, t;
  unsigned char sk_psd[PYR_N];
...
int test_BDS_update_treehash_tree_random(unsigned int max_height) {
  int res = 0;
  size_t j, k;
  uint32_t s, t;
  unsigned char sk_psd[PYR_N];
  unsigned char sk[BDS_BYTES];
  unsigned char sk_c[BDS_BYTES];
  unsigned char sk_jazz[BDS_BYTES];
  state_type state_c;
  stack_type stack_c[1];
  ...
  random_test_bytes(sk, BDS_BYTES);
  random_test_bytes(sk_psd, PYR_N);
...
#if 1
  res |= test_BDS_update_treehash_tree();
  res |= test_BDS_update_treehash_node();
#endif

  res |= test_BDS_update_treehash_node_random(NR_TEST);
  res |= test_BDS_update_treehash_tree_random(MAX_HEIGHT);
