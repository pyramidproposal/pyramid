...
/* Generates the specified number of random 32-bit blocks, starting at the position of the given pointer */
void random_test_32bit_blocks(uint32_t* r, size_t count)
{
	FILE* urandom = fopen("/dev/urandom", "r");
	if (fread(r, 4, count, urandom) != count) perror("Error in function random_test_32bit_blocks");
	fclose(urandom);
}
